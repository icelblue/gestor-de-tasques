...
function Index() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      ...
      {/* Header — títol estil "outlined" inspirat en el portfolio */}
      <header
        className="sticky top-0 z-50 flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3 sm:px-6 sm:py-5"
      ...
      >
        <div className="min-w-0">
          ...
          >
            ...
            <span
              style={{
                color: "transparent",
                WebkitTextStroke: "1.5px var(--muted-foreground)",
              }}
            >
              {T.headerOutline}
            </span>
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={openNew}
            className="rounded-sm px-3 py-2 font-mono text-xs font-semibold"
      ...
      {/* Content */}
      <div className="grid gap-px gt-content" style={{ background: "var(--border)" }}>
        <main style={{ background: "var(--background)" }}>
          <div className="border-b p-4" style={{ borderColor: "var(--border)" }}>
            ...
            {(tagFilter || dayFilter || filter !== "all") ? (
              <button
                onClick={() => { setTagFilter(null); setDayFilter(null); setFilter("all"); }}
      ...
      {modal === "import" ? (
        <ImportModal T={T} onClose={() => setModal(null)} onImport={(ts) => tasksStore.append(ts)} />
      ) : null}
      {modal === "report" ? (
        <ReportModal T={T} lang={lang} tasks={tasksStore.tasks} onClose={() => setModal(null)} />
      ) : null}
      ...
      {modal === "backup" ? (
        <BackupModal
          ...
          onReplace={(ts) => tasksStore.replaceAll(ts)}
          onClear={() => tasksStore.replaceAll([])}
        />
      ) : null}
    </div>
  );
